...
animatePath <- function(proc_obj, recs, out_dir, background = NULL,
                        background_ylim = c(41.48, 45.9),
                        background_xlim = c(-84, -79.5),
                        ffmpeg = NULL, plot_control = NULL,
                        int_time_stamp = 86400,
                        ani_out = NULL, frame_delete = FALSE ){
  setDT(procObj)
  setDT(recs)

# try calling ffmpeg if ani_out is not NA.  If ani_out is not NA, and ffmpeg is not
# present, then stop with warning.

if(!is.null(ani_out)){
  cmd <- ifelse(grepl("ffmpeg.exe$",ffmpeg) | is.null(ffmpeg), ffmpeg, paste0(ffmpeg,"\\ffmpeg.exe"))
  cmd <- ifelse(is.null(ffmpeg), 'ffmpeg', cmd)	
  ...
  if(!is.null(plot_control)){
    ...
    setDT(plot_control)
    procObj <- merge(proc_obj, plot_control, by.x=c("animal_id", "type"),
...
rng <- as.POSIXct(trunc(range(proc_obj$bin_stamp), units = "days"),
                  tz = "GMT")
