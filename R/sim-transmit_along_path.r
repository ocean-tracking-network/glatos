...
transmit_along_path <- function(path = NA, 
  ...
  if(!inherits(path, c("data.frame", "sf", "sfc", "SpatialPointsDataFrame"))) 
    stop("Input 'path' must be of class 'data.frame', 'sf', 'sfc', ",
  ...
  if(inherits(path, c("sf", "sfc"))) {
    if(!("POINT" %in% sf::st_geometry_type(path)))
      stop("Input object 'path' must contain geometry of type 'POINT' when ",
           "class is 'sf' or 'sfc'.")
    path_sf <- path
  ...
  if(nrow(trns) > 0){
    ...
    warning("Simulation resulted in no transmissions. Double check input ",
            "arguments.")
  }
  
  if(sp_out) return(trns_sf)
