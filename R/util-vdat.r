...
vdat_convert <- function(src, 
  ...
  stopifnot("Input 'vdat_exe_path' must be character or NULL" = 
  ...
  if(all(src_type == "dir")){
    ...
    if(nrow(ftp) == 0) {
      warning(paste0("No VRL or VDAT files were ",
                     "found in 'src': \n ",
                     paste(src, 
                           collapse = "\n ")))
  ...
  for(i in 1:nrow(ftp)){
    ...
    # or both
    vdat_call <- with(ftp, 
                      paste0("convert --format=", output_format, " \"", 
                        src_file[i], 
    ...
    msg_i <-  suppressWarnings(system2(command = vdat_cmd, 
                                       args = vdat_call, 
                                       stdout = TRUE, 
                                       stderr = TRUE))
...
get_local_vdat_version <- function(vdat_exe_path = NULL){
  ...
  if(grepl("^vdat-", vdat_version)) {
    vdat_version_out <- list(version = strsplit(vdat_version, "-")[[1]][2],
