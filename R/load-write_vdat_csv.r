...
write_vdat_csv <- function(vdat, 
  ...
  #Check input class
  if(!inherits(vdat, "vdat_list")) stop("Input 'vdat' must have class ",
                                        "'vdat_list'")
  
  # Check and/or make output file path
  
  # Is path or file?
  out_file <- if(is.null(out_file)) getwd() else 
  ...
  out_file_ext <- 
    data.table::fcase(output_format == "csv.fathom", ".csv",
  ...
  if(out_file_type == "dir"){
    ...
    out_file <- gsub(paste0("\\.", out_file_ext_in, "$"), 
                     out_file_ext,
                     out_file)
  }
  ...
  src_version <- paste0("VEMCO DATA LOG,", 
  ...
  vdat_lines_body <- setNames(object = vector("list", length(record_types)), 
                              record_types)
  
  vdat_lines_header <- setNames(object = vector("list", length(record_types)), 
                                record_types)
  ...
  for(i in 1:length(record_types)){
    ...
    if("Time Correction (s)" %in% names(x_i)){
      ...
      x_i[, `Time Correction (s)` := format(round(`Time Correction (s)`, 
                                                digits = 9), 
                                          nsmall = 9,
                                          trim = TRUE,
                                          drop0trailing = FALSE,
      ...
      x_i[`Time Correction (s)` == "0.000000000", 
          `Time Correction (s)` := "0"]
    }
    
    if("Ambient (deg C)" %in% names(x_i)){
      
      x_i[, `Ambient (deg C)` := format(round(`Ambient (deg C)`, 
                                              digits = 1), 
                                        nsmall = 1,
                                        trim = TRUE,
                                        drop0trailing = FALSE,
    ...
    if("Ambient Min (deg C)" %in% names(x_i)){
      ...
      x_i[, `Ambient Min (deg C)` := format(round(`Ambient Min (deg C)`, 
                                                  digits = 2), 
                                            nsmall = 2,
                                            trim = TRUE,
                                            drop0trailing = FALSE,
    ...
    if("Ambient Max (deg C)" %in% names(x_i)){
      ...
      x_i[, `Ambient Max (deg C)` := format(round(`Ambient Max (deg C)`, 
                                                  digits = 2), 
                                            nsmall = 2,
                                            trim = TRUE,
                                            drop0trailing = FALSE,
    ...
    if("Ambient Mean (deg C)" %in% names(x_i)){
      ...
      x_i[, `Ambient Mean (deg C)` := format(round(`Ambient Mean (deg C)`, 
                                                   digits = 2), 
                                             nsmall = 2,
                                             trim = TRUE,
                                             drop0trailing = FALSE,
    ...
    if("Internal (deg C)" %in% names(x_i)){
      ...
      x_i[ , `Internal (deg C)` := format(round(`Internal (deg C)`, 
                                                digits = 1), 
                                          nsmall = 1,
                                          trim = TRUE,
                                          drop0trailing = FALSE,
    ...
    vdat_lines_header[[i]] <- paste(gsub("record_type", 
                                         record_type_i_desc,
                                         txt_cols),
                                    collapse = ",")
    ...
    if(record_type_i == "EVENT"){
      ...
      vdat_lines_header[[i]] <- gsub(paste(event_colnames_to_drop, 
                                           collapse = "|"),
                                     "",
                                     vdat_lines_header[[i]])
    }
    ...
    if(record_type_i == "PING"){
      ...
      vdat_lines_header[[i]] <- gsub(paste(ping_colnames_to_drop, 
                                           collapse = "|"),
                                     "",
                                     vdat_lines_header[[i]])
    }
  ...
  vdat_lines_header3 <- paste0("RECORD TYPE", 
  ...
  if(output_format == "csv.fathom.split"){
    ...
    for(i in 1:length(vdat_lines_body)){
      ...
      if(nrow(vdat_lines_body[[i]]) > 0 | include_empty){
        out_file_i <- file.path(out_file,
                                paste0(names(vdat_lines_body[i]),
                                       ".csv"))
      
        vdat_out_i <- data.table::data.table(c(src_version, 
                                               "",
                                               vdat_lines_header2[[i]],
                                               vdat_lines_body[[i]]$txt))
        
        data.table::fwrite(x = vdat_out_i,
                           file = out_file_i,
                           col.names = FALSE,
                           quote = FALSE)
...
format_POSIXt <- function(x, digits = 0, drop0trailing = TRUE){
  ...
  stopifnot("'digits' must be >= 0 and <= 14" = 
              data.table::between(digits, 0, 14))
  
  # Coerce to POSIXlt
  xlt <- if(inherits(x, "POSIXct")) as.POSIXlt(x) else 
    if(inherits(x, "POSIXlt")) x else 
  ...
  frac_sec <- if(digits > 0) do.call(format,
                                     c(list(x = xlt$sec %% 1),
                                       drop0trailing = drop0trailing,
                                       trim = TRUE)) else character()
