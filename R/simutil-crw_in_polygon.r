 ...
 crw_in_polygon <- function(polyg=data.frame(x=c(0,0,150000,150000),
   ...
   if(any(is.na(initPos))){
		 ...
		 while(inPoly == FALSE){
			 ...
			 inPoly <- sp::point.in.polygon(init[1],init[2],polyg$x,polyg$y)
	  ...
	  if(all(!is.na(initPos))) {
		init <- initPos
		if(sp::point.in.polygon(init[1],init[2],polyg$x,polyg$y)==0) 
	  ...
	  for(i in 1:nwin){
		  ...
		  #check if in polygon
		  inPoly <- sp::point.in.polygon(path.fwd$x[rows.i],path.fwd$y[rows.i],
		    polyg$x,polyg$y)
		  ...
		  while(!all(inPoly)){
			  ...
			  constrainThetaToPolygon <- function(x,y,len,polyg,theta){
					...
					bar$inPoly <- sp::point.in.polygon(bar$x,bar$y,polyg$x,polyg$y)
