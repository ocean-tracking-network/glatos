...
# first create a dataframe  of distances to iterate over
...
1) With fewer data points a third order polynomial often fits the data better, however, this does not mean that neither a logit or probit model should not be assessed.
...
We can calculate our preliminary detection efficiency also in R instead of using either the two software produced by Innovasea. To do so we need three things, the detection data, the receiver location data, and the transmitter deployment data. 

For the purpose of the vignette, I am going to manufacture this data. 
