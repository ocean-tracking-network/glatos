...
Estimated detection probability dropped to about `r#round(dph*100,1)`% of fish after losing a receiver, so our final decision might depend on how likely we are to lose a receiver during the study.
...
library(sf)
bar <- st_as_sfc(greatLakesPoly)

one <- st_union(bar, by_feature = FALSE)

test <- st_crop(one, c(xmin=-84.883971, xmax=-79.696182, ymin=42.932699, ymax=46.474450))
plot(test)


str(st_coordinates(test))
