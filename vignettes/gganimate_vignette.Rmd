...
from `glatos`, clean up the data, create a figure of
...
lakes <-  crop(greatLakesPoly, extent(-84.8, -79.6, 41.3, 46.6))
plot(lakes, col = "grey")
...
# brind in receiver file and convert to sf spatial object
...
In the next line of code, we create a transition layer from `lakes`.
...
Now we will animate the plot using `gganimate`
...
animate(out, nframes = 100, fps = 10)

```

Notice the only change in the code above is the addition of a
`transition_time` and `ggtitle` component.  The resulting animation may be
customized by specifying arguments to the `animate` function.  Please
...
make_transition <- function (in_file, output = "out.tif", output_dir = NULL, res = c(0.1, 
    0.1), all_touched = TRUE) 
{
    ...
    burned <- gdalUtils::gdal_rasterize(temp_dir, dst_filename = path.expand(file.path(output_dir, 
        output)), burn = 1, tr = res, output_Raster = TRUE, at = all_touched)
