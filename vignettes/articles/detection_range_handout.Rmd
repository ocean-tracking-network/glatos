...
```{r fig1, fig.width=6, fig.height=6, echo=FALSE, warning=FALSE, message=FALSE, fig.cap=cap1}
library(sf)
library(glatos)
library(data.table)
library(ggplot2)

# get sample range detection data from glatos pkg
...
```{r, ,fig.cap=cap2,echo=TRUE,fig.width=6,fig.height=4}
...
```{r warning=FALSE,echo=TRUE,message=FALSE,fig.cap=cap3,fig.width=8,fig.height=8,fig.keep="first",fig.show="asis"}
...
```{r fig.keep="last",fig.width=6,fig.height=6,message=FALSE,fig.cap=cap4,fig.show="asis"}
# add tag-date name
...
```{r echo=TRUE,fig.height=8,fig.width=8,fig.cap=fig_cap6}
...
```{R, echo = TRUE, fig.height=8,fig.width=8,fig.cap=fig_cap_grid}
...
```{R, echo = TRUE, fig.height=8,fig.width=8,fig.cap=fig_cap_sim}
sp::plot(LH_latlon, col = "lightgrey", border = "grey")
plot(st_geometry(path), type = "o", pch = 20, col = "red", add = TRUE)
plot(st_geometry(grid_10km), pch = 20, col = "black", add = TRUE)
