...
The R package *glatos* (version 0.2.6 and earlier) contains five functions 
designed to load data files in standardized formats from the Great Lakes 
Acoustic Telemetry Observation System (GLATOS), the Ocean Tracking Network 
(OTN), and VEMCO. Each data loading function:  

1. loads data into an R session consistently and efficiently using the best 
available methods and  

2. returns an object that meets the requirements of relevant package functions. 
...
- _**read_glatos_workbook**_
 : reads data from a GLATOS project-specific 
      MS Excel workbook (\*.xlsm file) and returns a list of class *glatos_workbook*
...
and will therefore be compatible with *glatos* functions.
...
*glatos*. Functions discussed in this vignette can be used to load 
data files from other sources by manually creating required data structure
and specifying non-default arguments in function calls.  
the [Data Requirements vignette](data_dictionary.html) provides information about data type for required fields.
...
examples using functions from contributed packages. Specifically, boxed tips 
...
is, to our knowledge, the fastest way to coerce timestamps strings to POSIXct. 
Otherwise, we provide no examples using *tidyverse* (*dplyr*, *ggplot2*, 
*readr*) functions, simply because some of us have not yet drunk the kool-aid. 
A future version of this vignette may include examples using those packages.

Something here about box example code... 1) if you run it, make sure attach 
the packages, (2) setDT() at first call, (3) go all-in, don't alternate.
