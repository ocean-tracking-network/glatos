...
The R package *glatos* (version 0.2.6 and earlier) contains five functions (described below) designed to load data files in standardized formats from the Great Lakes Acoustic Telemetry Observation System (GLATOS), the Ocean Tracking Network (OTN), and VEMCO. Strictly speaking, there are no specific data requirements for the *glatos* package as a whole, but each data loading function does require specific fields and data types in order to ensure that data are loaded efficiently and consistently. Required columns for each function are described in each function-specific help file (e.g., `?read_glatos_detections`) and the [Data Requirements vignette](data_dictionary.html) provides information about data type for required fields. Functions discussed in this vignette can be used to load data
...
GLATOS receiver data in R and show how to manually coerce a
...
str(walleye_detections, vec.len = 1)
```
...
str(blue_shark_detections, vec.len = 1)
```
...
### Other formats - loading detections exported from a VUE database
...
> \center __data.table tip:__ Use _fread_ instead of _read.csv_.
\center  
>  
> `dtc <- fread(csv_file)`  
>  
...
below (e.g., `names(dtc)[1] <- "detection\_timestamp\_utc"`) but use of 
...
> \center __data.table tip:__ Use _setnames_ to change column names.
\center  
>  
...
str(dtc$detection_timestamp_utc, vec.len = 1)
```

***

> \center __lubridate tip:__ Use _fast\_strptime_ to set timestamps.
\center  
>  
...
> \center __data.table tip:__ Use _:=_ to add or modify columns.
\center  
>  
> `#make new column "receiver_sn"; parse from "Receiver"`
...
> \center __data.table tip:__ Use `:=(...)` to 'set' (add or modify) more than one column.
\center  
>  
> `#make new column "transmitter_codespace"; parse from "Transmitter"`
...
> `       by = "Transmitter"]                                        `
>  
> Note that curly braces are used to allow an intermediate variable (tx)
> that is not included in the result.
> 
...
> \center __data.table tip:__ Use _between_ to simplify this query.
\center  
>  
> `#subset deployments between receiver deployment and recovery (omit others)`
...
### Loading data from other formats - VEMCO VUE (the *data.table* way)
...
### Loading from other formats - VEMCO VUE 
...
# round(t1[["elapsed"]] / t3[["elapsed"]]) times faster than `as.POSIXct()`. Again, the 
