# glatos: A package for the Great Lakes Acoustic Telemetry Observation System

Functions useful to members of the Great Lakes Acoustic Telemetry
Observation System https://glatos.org; many more broadly relevant to
simulating, processing, analysing, and visualizing acoustic telemetry
data.

## See also

Useful links:

- <https://github.com/ocean-tracking-network/glatos>

- Report bugs at
  <https://github.com/ocean-tracking-network/glatos/issues>

## Author

**Maintainer**: Christopher Holbrook <cholbrook@glfc.org>

Authors:

- Christopher Holbrook <cholbrook@glfc.org>

- Todd Hayden

- Thomas Binder

- Jon Pye

Other contributors:

- Mike O'Brien \[contributor\]

- Alex Nunes \[contributor\]

- Benjamin Hlina \[contributor\]

- Angela Dini \[contributor\]

- Ryan Gosse \[contributor\]
