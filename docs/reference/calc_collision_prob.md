...
Estimate (by simulation) probability of collision for co-located
telemetry transmitters with pulse-period-modulation type encoding
...
- maxTags:
  ...
  A numeric scalar with maximum number of co-located transmitters
  (within detection range at same time).

- nTrans:

  A numeric scalar with the number of transmissions to simulate for each
...
- expDetsPerHr:
  ...
  Expected number of detections per hour assuming perfect detection
  probability, given the number of tags within detection range
...
Calculates the detection probability associated with collision, given
delay range (delayRng), burst duration (burstDur), maximum number of
co-located tags (maxTags), and number of simulated transmission per tag
(nTrans). The simulation estimates detection probability due only to
collisions (i.e., when no other variables influence detection
probability) and assuming that all tags are co-located at a receiver for
...
\
Binder, T.R., Holbrook, C.M., Hayden, T.A. and Krueger, C.C., 2016.
