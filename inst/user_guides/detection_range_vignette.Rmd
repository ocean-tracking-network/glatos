...
10.1111/2041-210X.12996.]  In the first chunk, we make the 5 km and
2.5 km grids by adapting the code from the previous example.
...
# make 2.5 km grid
grid_2.5km <- st_intersection(LH, st_make_grid(LH, cellsize = 2500, what = "centers"))
grid_2.5km <- st_transform(grid_5km, crs = 4326)
grid_2.5km_sp <- as(grid_2.5km, "Spatial")
...
# do random walk in LH polygon of virtual fish starting in Saginaw Bay
  paths[[i]] <- crw_in_polygon(LH_latlon, theta = c(0,25),
                             ...
                             nsteps = 30, initPos = c(-83.6, 43.8), sp_out = FALSE)
}
...
foo <- paths[, transmit_along_path(.SD, vel = 0.5, delayRng = c(60,180), sp_out = FALSE), by = virt_fish]
...
# plot path, grid and points
sp::plot(LH_latlon, col = "lightgrey", border = "grey")
points(paths[virt_fish == 1, get("x")], paths[virt_fish == 1, get(c("y"))], type = "o", pch = 20, col = "red")

points(foo[virt_fish == 1, get("x")], foo[virt_fish == 1, get("y")], pch = 20, col = "blue")
