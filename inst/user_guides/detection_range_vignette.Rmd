...
and create a grid of virtual receivers that are spaced 30 km apart.
...
grid_30km <- st_intersection(LH, st_make_grid(LH, cellsize = 30000, what = "centers"))

# convert back to WGS84-lat/lon
grid_30km <- st_transform(grid_30km, crs = 4326)

# convert coordinates to sp for future use 
grid_30km_sp <- as(grid_30km, "Spatial")
...
path <- crw_in_polygon(LH_latlon, theta = c(0,25), stepLen= 10000, initHeading = 0, nsteps = 30)

```

Now it is time to bring together fish track, grid and outline to make
...
mydtc <- detect_transmissions(trnsLoc = tag_trans, recLoc = grid_30km_sp, 
                                detRngFun = edrf )

#points(mydtc, col = "blue", cex = 2, lwd = 2)
