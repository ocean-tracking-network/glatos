...
First, we need to load libraries, read in the detection data file, and do a
...
mean(diff(sim$etime))
det_interval <- mean(diff(sim$etime))
...
virtual fish was detected `r nrow(mydtc)` times on `r
length(unique(mydtc$recv_id))` receivers.  The mean time interval
between successive detections was `r det_interval` seconds and the
mean elapsed time difference between first detections on successive
receivers is `r f_int` seconds.
...
performance of a 5 km grid with a 2.5 km grid.  We will keep the tag
...
# make 10 km grid
grid_10km <- st_intersection(LH, st_make_grid(LH, cellsize = 5000, what = "centers"))
grid_10km <- st_transform(grid_10km, crs = 4326)
grid_10km_sp <- as(grid_10km, "Spatial")

# make 5 km grid
grid_5km <- st_intersection(LH, st_make_grid(LH, cellsize = 2500, what = "centers"))
...
rec_by_dtc <- mean(out$t_diff, na.rm = TRUE)
...
10 km grid (`r rec_by_dtc`), we are going to
...
detections on successive receivers was `r rec_by_dtc` for a 10 km grid
and `r rec_by_dtc_sim2` for a 5 km grid.  Likewise, the mean time
between detections was `r det_interval` seconds for the 10 km grid and
`r det_interval_sim2` for the 5 km grid.  As you can see, the 5 km
