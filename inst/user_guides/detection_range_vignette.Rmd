...
tag_trans <- transmit_along_path(path_df, vel= 0.5, delayRng = c(60, 180) )
...
## paths <- rbindlist(paths, idcol = "virt_fish")

## # calculate transmissions along path for each virtual fish
## foo <- paths[, transmit_along_path(.SD[,c("x", "y")], vel = 0.5, delayRng = c(60,180), sp_out = FALSE), by = virt_fish]


## #tag_trans <- transmit_along_path(path, vel= 0.5, delayRng = c(60, 180) )


## # calculate transmissions for each virtual fish  
## transmissions <-  vector("list", length = 100)
## paths <- as.data.frame(paths)

## for(i in 1:100){
##   virt_fish.i <- paths[ paths$virt_fish == i, c("x", "y")]
##   transmissions[[i]] <- transmit_along_path(virt_fish.i, vel = 0.5,
##                                             delayRng = c(60,180), sp_out = FALSE)
## }

# plot path, grid and points
## sp::plot(LH_latlon, col = "lightgrey", border = "grey")
## points(paths[[1]]$x, paths[[1]]$y, type = "o", pch = 20, col = "red")
## points(trans[[1]]$x, trans[[1]]$y, pch = 20, col = "blue")
## plot(st_geometry(grid_10km), pch = 20, col = "black", add = TRUE)
...
sim <- rbindlist(foo, fill = TRUE, idcol = "virt_fish")
