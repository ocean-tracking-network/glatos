...
```
```{r, echo=TRUE, fig.cap=fig.cap1, fig.height=8,fig.width=8,tidy=TRUE,warning=FALSE,message=FALSE}
...
# convert to Great Lakes Coordinate reference system
# this is needed in order to crop out lakes Huron and Erie.
lakes <- st_transform(lakes, crs = 3174)
  
# crop polygon to extract Lake Huron and Lake Erie
lakes <-  st_crop(lakes, xmin = 971159.58, xmax = 1371224.26, ymin = -525362.49, ymax= 1126020.41)
plot(st_geometry(lakes), col = "grey")

# convert lakes object back to 4326 crs
...
# remove receivers that do not have a recovery_date_time (lost receivers)
...
pos2 <- interpolate_path(det, trans = tran$transition, lnl_thresh=0.99, out_class = "data.table")

```

```{r, echo = FALSE,tidy=TRUE,message=FALSE}

dump <- capture.output( {pos2 <-interpolate_path(det, trans = tran$transition, lnl_thresh=0.99, out_class = "data.table")})
...
# set data.table key and order bin_timestamp from smallest to largest
setkey(int, bin_timestamp)

```

In the next code chunk, we use a `data.table` overlap-join to add the
unique timestamps extracted from `pos2` in the last line of code above
(named `int`) to the receiver object.  We use receiver deployment and
...
receiver deployment and recovery in the animation.  This step is easy
with `data.table` and would be a bit complicated using base R
functions.

```{r, echo=TRUE,tidy=TRUE}

# calculate start and end dates for bin_timestamp
# start is bin_timestamp, end is next bin_timestamp, except for last bin where start and end is the same value
int <- int[, c("start","end") := list(
  bin_timestamp, shift(bin_timestamp, type = "lead",
                       fill = max(bin_timestamp)))]

# set data.table keys
# this is required by data.table for join
setkey(int, start, end)

# do overlap join
recs <- foverlaps(recs, int, by.x = c("deploy_date_time", "recover_date_time"),
                  by.y = c("start", "end"))

# clean up recs object.
# Only need bin_timestamp, deploy_lat, deploy_long columns for animation
recs <- recs[, c("bin_timestamp", "deploy_long", "deploy_lat")]
