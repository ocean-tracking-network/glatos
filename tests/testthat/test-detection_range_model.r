...
test_that("Test if polynomial model for model_frame = 'data_frame', is correct", {
  ...
  m <- expect_warning(
    detection_range_model(avg_percent ~ -1 + distance_m + I(distance_m ^ 2) + 
...
test_that("Test if polynomial model for model_frame = 'matrix', is correct", {
  m1 <- expect_warning(
    detection_range_model(avg_percent ~ -1 + poly(distance_m, 3, raw = TRUE) + 
                                offset(intercept), 
                              data = sample_detection_efficiency, 
                              p = c(50), 
                              link = "polynomial",
...
test_that("Test if logit model is correct", {
  m2 <- expect_warning(
    detection_range_model(avg_percent_d ~ distance_m,
                              data = sample_detection_efficiency, 
                              p = c(50),
                              link = "logit",
                              summary_stats = TRUE)
