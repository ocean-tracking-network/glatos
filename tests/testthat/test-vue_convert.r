...
test_that("vue_convert catches bad inputs", {
  ...
  # src is missing file
  expect_error(vue_convert(tempfile()), 
               "Input 'src' not found:")
  
  # src is empty dir
  expect_warning(vue_convert(temp_vrl_dir2), 
  ...
  expect_error(vue_convert(c(temp_vrl_files2[1], tempdir())),
               paste0("Input arg 'src' must contain one or more ",
                      "files or directories, but not both\\."))
  
  
  # missing out_dir
  expect_error(vue_convert(temp_vrl_files2[1],
  ...
  expect_message(vue_convert(temp_file2, overwrite = TRUE),
  ...
  expect_warning(vue_convert(temp_file2, overwrite = TRUE, diagn = TRUE),
...
test_that("check_vue works", {
  ...
  expect_equal(check_vue("C:/Program Files (x86)/Vemco/VUE"), 
               "C:/Program Files (x86)/Vemco/VUE/VUE.exe")
  
  # path contains vue.exe
  expect_equal(check_vue("C:/Program Files (x86)/Vemco/VUE/VUE.exe"), 
               "C:/Program Files (x86)/Vemco/VUE/VUE.exe")
  
  # vdat.exe not found
  expect_error(check_vue(tempdir()),
...
test_that("get_local_vue_version works", {
  ...
  expect_type(temp_vue_version <- get_local_vue_version(),
              "list")
  
  expect_equal(names(temp_vue_version), 
               c("version", 
                 "long_version"))
