...
test_that("vdat_convert catches bad inputs", {
  ...
  # src is missing file
  expect_error(vdat_convert(tempfile()), 
               "Input 'src' not found:",
               fixed = TRUE)
  
  # src is empty dir
  expect_warning(vdat_convert(temp_vrl_dir2), 
  ...
  expect_error(vdat_convert(c(temp_vrl_files2[1], tempdir())),
                 paste0("Input arg 'src' must contain one or more ",
                        "files or directories, but not both."),
                 fixed = TRUE)
  
  
  # missing out_dir
  expect_error(vdat_convert(temp_vrl_files2[1],
  ...
  expect_error(vdat_convert(temp_vrl_files2[1],
               ...
               paste0("Input 'out_dir' must be NULL, length one, or ",
                      "same length at 'src'."),
               fixed = TRUE)
  ...
  expect_error(vdat_convert(temp_vrl_files2[1],
               ...
               paste0("'out_dir' cannot contain full paths to files; ",
                "only directories."),
               fixed = TRUE)
  ...
  expect_message(vdat_convert(temp_file2, overwrite = TRUE),
  ...
  expect_warning(vdat_convert(temp_file2, overwrite = TRUE, diagn = TRUE),
                 "Error converting bad_input.vrl :",
                 fixed = TRUE) |>
    expect_message("Converting 1 VRL/VDAT file(s) to Fathom CSV...", 
                   fixed = TRUE) |>
      expect_output() |> #progressbar
        expect_message("Done. 0 of 1 file(s) written to disk.",
                       fixed = TRUE) |>
...
test_that("check_vdat works", {
  ...
  expect_equal(check_vdat("C:/Program Files/Innovasea/Fathom/vdat.exe"), 
               "C:/Program Files/Innovasea/Fathom/VDAT.exe")
  
  # vdat.exe not found
  expect_error(check_vdat(tempdir()),
...
test_that("get_local_vdat_version works", {
  ...
  expect_type(temp_vdat_version <- get_local_vdat_version(),
                  "list")
  
  expect_equal(names(temp_vdat_version), 
                 c("version", 
                   "long_version"))
  ...
  expect_error(get_local_vdat_version(tempdir()), 
               "VDAT.exe not found at specified path.",
               fixed = TRUE)
...
test_that("get_local_vdat_template works", {
  ...
  expect_equal(names(temp_vdat_schema),
   c("VEMCO DATA LOG", "ATTITUDE", "BATTERY", "CFG_CHANNEL", "CFG_RECEIVER_HR3", 
