...
set.seed(33)
dtc_spin_spout <- detect_transmissions(
  trnsLoc = tr_sf,
  recLoc = recs_sf,
...
set.seed(33)
dtc_spin_dfout <- detect_transmissions(
  trnsLoc = tr_sf,
  recLoc = recs_sf,
  detRngFun = function(x) 0.5,
  sp_out = FALSE,
...
# spatial detection output - 50% constant detection prob
set.seed(33)
dtc_dfin_spout <- detect_transmissions(
  trnsLoc = tr_df,
  recLoc = recs_df,
  detRngFun = function(x) 0.5,
...
# non-spatial detection output - 50% constant detection prob
set.seed(33)
dtc_dfin_dfout <- detect_transmissions(
  trnsLoc = tr_df,
  recLoc = recs_df,
  detRngFun = function(x) 0.5,
  inputCRS = 4326,
...
# Expected values
dtc_dfout_shouldBe <- readRDS(file.path(
  testdata_dir,
  "test-detect_transmissions-dtc_dfout.rds"
))
dtc_spout_shouldBe <- readRDS(file.path(
  testdata_dir,
